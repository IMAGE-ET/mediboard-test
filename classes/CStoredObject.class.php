...
class CStoredObject extends CModelObject {
    ...
    foreach ($this->_props as $name => $prop) {
      if ($name[0] !== '_') {
        if (!property_exists($this, $name)) {
          trigger_error("La sp�cification cible la propri�t� '$name' inexistante dans la classe '$this->_class'", E_USER_WARNING);
        }
        else {
          $value = $this->$name;
  ...
  function delete() {
    ...
    foreach ($this->_backSpecs as $backSpec) {
      ...
      // Ne pas utiliser loadMatchingList au cas où l'objet traite mal les champs initialisés
      $where = array();
      $where[$backField] = "= '$this->_id'";
