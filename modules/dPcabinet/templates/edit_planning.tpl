...
<script>
  ...
  resetPlage = function(id) {
    ...
    if ($V(oForm["consult_id_"+id]) && !$V(oForm["cancel_"+id])) {
      $V(oForm["cancel_"+id], "");
      SystemMessage.notify("Cette consultation sera annul�e");
    }
    else {
      SystemMessage.notify("Cette consultation ne sera pas cr��e");
      $V(oForm["_consult"+id], "");
          ...
          <table class="form">
            ...
            <tr id="viewPatient" {{if $consult->_id && $consult->patient_id==0}}style="display:none;"{{/if}}>
              ...
              <td>
                ...
                {{else}}
                  <div class="info text">
                    Consultation de sejour, dissocier du s�jour pour changer le patient ou changer le patient du s�jour
                  </div>
                {{/if}}
                    ...
                    <td>
                      ...
                      {{if $following_consultations|@count}}
                        <button class="agenda button " id="buttonMultiple" type="button" onclick="PlageConsultSelector.init(1, 1);" id="buttonMultiple">
                          ...
                          {{else}}
                            {{$following_consultations|@count}} consultation{{if $following_consultations|@count > 1}}s{{/if}} ult�rieure{{if $following_consultations|@count > 1}}s{{/if}}
                          {{/if}}
                        </button>
                      {{else}}
                  ...
                  <tr>
                    ...
                    <td>
                      {{mb_field object=$consult field="premiere" typeEnum=checkbox}} {{if $consult->_consult_sejour_out_of_nb}}
                        <strong>S�ance {{$consult->_consult_sejour_nb}} / {{$consult->_consult_sejour_out_of_nb}}</strong>
                      {{/if}}
                    </td>
                  </tr>
