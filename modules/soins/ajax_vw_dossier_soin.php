...
// Recuperation du sejour_id si seulement l'object est pass�
...
if ($group->_id != $sejour->group_id) {
  CAppUI::stepAjax("Ce s�jour n'est pas dans l'�tablissement courant", UI_MSG_WARNING);
  return;
}
...
if (CModule::getActive("dPprescription")) {
  ...
  // Chargement des cis � risque
  $where = array();
  $where["risque"]    = " = '1'";
  $risques_cis = CProduitLivretTherapeutique::getCISList($where);

  // Chargement de la prescription � partir du sejour
  ...
  if ($object_id && $object_class) {
    ...
    if ($prescription->_id) {
      ...
      if (in_array($chapitre, array("all_med", "all_chaps"))) {
        ...
        // Chargement des lignes d'�l�ments
      ...
      elseif ($chapitre == "inscription") {
        // Chargement des inscriptions effectu�es
...
if ($object_id && $object_class) {
  ...
  if ($chapitre) {
    ...
    if (CModule::getActive("dPprescription")) {
      // Multiple prescriptions existante pour le s�jour (Fusion des prescriptions)
